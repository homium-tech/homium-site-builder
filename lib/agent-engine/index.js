...
class AgentEngine {
  ...
  executeTurn({ sessionId, message, engine }) {
    ...
    const history = this.sessionStore.getHistory(session.id);
    ...
    if (isFirstTurnForEngine) {
      ...
      const historyBlock = history
        ? `\n\nHISTORIAL DE CONVERSACIÓN (para continuidad — continúa exactamente desde donde quedaste):\n${history}\n`
        : '';
      promptText = `[DIRECTIVA HOMIUM: Operas de forma autónoma sin skills externas. Tono 100% neutral, sobrio y pragmático. Prohibido usar frases de adulación o cortesía artificial. Comienza directo con el siguiente paso. 5 fases canónicas estrictas, termina en Fase 5 con prototipo vanilla HTML/CSS/JS. PROHIBIDO Fase 6 o frameworks externos. Single-Question Rule activa: UNA sola pregunta por turno. Workspace: ${this.cwd}]${historyBlock}\nMensaje actual del usuario: ${message}`;
    }
